...
const handleSearchSubmit = (e) => {
  e.preventDefault();
  // Implement search logic here, e.g., call an API to fetch search results based on searchQuery
      ...
      <div className="menu-container relative ">
        <div className="mobile-menu-icon md:hidden " onClick={toggleMobileMenu}>
          ...
          <div className={`mobile-menu absolute top-full left-0 w-full bg-white shadow-md ${isMobileMenuOpen ? 'block' : 'hidden'} md:hidden `}>
            ...
            <ul className="menu flex gap-6 list-none m-0 p-0 text-sm font-medium text-gray-700 menu-items hidden md:flex ">
...
<div className={`search-results absolute top-full left-0 w-full bg-white shadow-md mt-1 rounded ${searchResults.length > 0 ? 'block' : 'hidden'} search-results-container `}>
  <ul className="search-results-list list-none m-0 p-2 text-sm text-gray-700 ">
    {searchResults.map((result, index) => (
      <li key={index} className="search-result-item py-1 hover:bg-gray-100 cursor-pointer ">
        {result}
      </li>
    ))}
  </ul>
   ...
   <div className="search-bar-container relative ">
      ...
      <input 
        ...
        className="search-input border rounded px-4 py-2 w-64 focus:outline-none focus:ring-2 focus:ring-blue-500 search-bar "
        value={searchQuery}
      ...
      <div className="mobile-search-container absolute top-full left-0 w-full bg-white shadow-md mt-1 rounded md:hidden ">
